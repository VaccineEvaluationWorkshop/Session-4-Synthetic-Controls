...
```{r}
 plot(d2$date,d2$Pneum,  #names of variable
...
```
Same for the 1-4 year olds

```{r, fig.width=5, fig.height=4}
      plots$groups[["1-4 years"]]$pred_full 
```

In contrast, we don't see a decline among older adults.
```{r, fig.width=5, fig.height=4}
      plots$groups[["65-79 years"]]$pred_full 
```
...
It is clear here why the unadjusted time trend model fails.
